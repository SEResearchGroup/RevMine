...
class KafkaClient:
    """
    Producer + Consumer réutilisable.
    Usage : instancier une fois au démarrage du service.
    """
    ...
    def publish(cls, topic: str, payload: dict, key: str = None):
        ...
        try:
            ...
            future.get(timeout=10)  # attend l'ack broker
    ...
    def start_consumer(
        ...
        """
        Démarre un consumer dans un thread background.
        handler(payload: dict, topic: str) → appelé pour chaque message.
        """
        def _consume():
            while True:
                try:
