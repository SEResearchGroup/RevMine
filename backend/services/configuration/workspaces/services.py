...
class GitAPIClient:
    ...
    def get(self, endpoint: str, params: Optional[Dict] = None, timeout: int = 15) -> requests.Response:
        """
...
class RepositoryService:
    ...
    @staticmethod
    def fetch_repositories(platform: str, token: str, url: Optional[str] = None) -> Dict:
        """
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {'success': False, 'message': f'API error: {response.status_code}', 'repositories': []}
    ...
    def _import_single(workspace: Workspace, repo_data: Dict) -> Repository:
        ...
        normalizer = (
            GitHubNormalizer() if workspace.platform == 'github' 
            else GitLabNormalizer()
        )
