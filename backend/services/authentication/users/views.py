...
class IntrospectView(APIView):
    """Validate a JWT for the API Gateway without moving auth logic into it."""
        ...
        try:
            ...
            if not email:
                ...
                if email_response.status_code == 200:
                    ...
                    email = (
                        primary_email["email"] if primary_email else emails[0]["email"]
                    )

            if not email:
                return Response(
                    {"error": "No email found"}, status=status.HTTP_400_BAD_REQUEST
                )
            ...
            user, created = User.objects.get_or_create(
                ...
                defaults={
                    ...
                    "last_name": (
                        ...
                        if user_data.get("name")
                        and len(user_data.get("name", "").split()) > 1
                        else ""
                    ),
                },
            )
