...
class AnalysisCreateView(APIView):
    ...
    def post(self, request):
        ...
        try:
            # Save CSV file
            file_path = default_storage.save(f"datasets/{csv_file.name}", csv_file)
            ...
            with transaction.atomic():
                ...
                analysis = Analysis.objects.create(
                    dataset=dataset, requested_charts=requested_charts, status="pending"
                )
    ...
    def _process_analysis(
        self, analysis, file_path, requested_charts, platform="gitlab"
    ):
        """
        ...
        try:
            ...
            chart_functions = {
                ...
                "mr_creation_timeline": lambda: plot_mr_creation_timeline(
                    df, "Creation_Date", "W"
                ),
            ...
            for chart_type in requested_charts:
                if chart_type in chart_functions:
                    try:
                        ...
                        static_image = self._generate_static_chart(
                            df, chart_type, result
                        )
            ...
            print(
                f"Total charts generated: {AnalysisResult.objects.filter(analysis=analysis).count()}"
            )
...
class AnalysisDetailView(APIView):
    ...
    def delete(self, request, analysis_id):
        ...
        try:
            if analysis.dataset.file_path and os.path.exists(
                analysis.dataset.file_path
            ):
