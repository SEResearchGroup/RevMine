...
from .devops_collectors import (
    GitHubActionsCollector,
    GitHubProjectsCollector,
    GitLabBoardsCollector,
    GitLabCICollector,
)
from .models import DevOpsCollectionJob
...
def _publish_notification(
    ...
    The notification microservice consumes the topic and pushes to the user
    over WebSocket (see `backend/services/notification/internal/kafka/consumer.go`).
    """
    if not job.user_id:
        return
    try:
        ...
        # Service running without kafka utils — silently skip.
        return
