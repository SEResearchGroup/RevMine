"""
Domain -- Metrics Engine
========================
Contains all metric computation methods for the analyze service.
Each analyze_* method takes a pandas DataFrame and an Analysis model
instance (for config/chart_type), and returns:

    {
        'chart_data':  dict,   # Chart.js / Recharts compatible payload
        'chart_image': str,    # Base64-encoded matplotlib PNG (optional)
        'statistics':  dict,   # Summary statistics
    }

No Django ORM queries. No file I/O. Pure computation.
"""
import pandas as pd
import numpy as np
import matplotlib
...
class MetricsEngine:
    """
    Domain engine for metric computation.

    Instantiate once per analysis run (or keep as a singleton).
    All analyze_* methods receive the DataFrame and an Analysis model
    instance and return a structured result dict.
    """
    def __init__(self):
