...
class Collection(models.Model):
    ...
    branch_name = models.CharField(
        max_length=255, null=True, blank=True, help_text="Branch to collect data from"
    )
    ...
    def can_resume(self):
        ...
        """
        if (
            self.status in ["paused", "failed"]
            and self.last_collected_item_id is not None
        ):
            return True
    ...
    def get_active_for_repository(cls, user_id, repository_id):
        ...
        return cls.objects.filter(
            user=user_id, repository_id=repository_id, status__in=cls.ACTIVE_STATUSES
        ).first()
...
class CleanedData(models.Model):
    ...
    start_date = models.DateField(
        null=True, blank=True, help_text="Start date for cleaning filter"
    )
    end_date = models.DateField(
        null=True, blank=True, help_text="End date for cleaning filter"
    )

    # Cleaning configuration
    filters = models.JSONField(default=dict, help_text="Cleaning filters configuration")
    selected_features = models.JSONField(
        default=list, help_text="List of feature IDs to include in statistics CSV"
    )
    ...
    # Statistics
    stats = models.JSONField(default=dict, help_text="Cleaning statistics")
    ...
    def __str__(self):
        return (
            f"CleanedData {self.id} for Collection {self.collection_id} ({self.status})"
        )
