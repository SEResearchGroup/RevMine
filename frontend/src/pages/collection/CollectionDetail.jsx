...
function CollectionDetail() {
  ...
  const fetchCollectionData = async () => {
    try {
      ...
      // Fetch repo info only for non-external collections
      if (workspaceId && workspaceId !== "0") {
        try {
          const reposRes = await workspaceService.getRepositories(workspaceId);
          const repo = reposRes.data.find((r) => r.id === parseInt(repositoryId));
          setRepository(repo);
        } catch {
          // Ignore – repository info is optional for display
        }
      }
    } catch (err) {
