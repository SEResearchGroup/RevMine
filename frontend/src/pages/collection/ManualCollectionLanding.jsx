...
import { Search, FolderGit2, ChevronDown, Filter, X, Settings2 } from "lucide-react";
import { workspaceService } from "../../services/api";
import RepositoryCard from "../../components/repositories/RepositoryCard";

const PAGE_SIZE = 20;

function ManualCollectionLanding() {
  ...
  const goToManualCollect = (repo) =>
    navigate(`/workspaces/${repo.workspace}/repositories/${repo.id}/collect?mode=manual`);
    ...
    <div className="min-h-screen p-4 sm:p-6">
      <div className="max-w-7xl mx-auto mb-4 sm:mb-6">
        <h1 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-2">
          <span className="text-blue-600">Collection</span> / Manual Collect
        </h1>
        <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-4 mb-6">
          ...
          <div>
            <p className="text-sm text-blue-900 font-medium">
              Pick a project to configure a manual collection.
            </p>
            <p className="text-sm text-blue-800/80 mt-1">
              You'll choose the branch, metrics, and date range yourself. If you'd
              rather describe the analysis in plain English, use{" "}
              <button
                onClick={() => navigate("/collection/intelligent")}
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-400 text-sm mt-1">
              Import repositories from your workspaces to see them here.
            </p>
            <button
              onClick={() => navigate("/workspaces")}
              className="mt-4 px-5 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm"
            >
              Go to Workspaces
            </button>
          </div>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {repositories.map((repo) => (
                <RepositoryCard
                  onClick={() => goToManualCollect(repo)}
                  key={repo.id}
                  repo={repo}
                  showWorkspace
                  onCollect={goToManualCollect}
                />
              ))}
            </div>
...
export default ManualCollectionLanding;
