    ...
    <div className="min-h-screen p-4 sm:p-6">
      <div className="max-w-7xl mx-auto mb-4 sm:mb-6">
        <h1 className="text-xl sm:text-2xl font-semibold text-gray-800 mb-6">
          <span className="text-blue-600">Data Sources</span> / Projects
        </h1>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {repositories.map((repo) => (
                <RepositoryCard
                  onClick={
                    (r) => navigate(`/workspaces/${repo.workspace}/repositories/${repo.id}/collect`)
                  }
                  key={repo.id}
                  repo={repo}
                  showWorkspace
                  onCollect={(r) =>
                    navigate(
                      `/workspaces/${r.workspace}/repositories/${r.id}/collect`
                    )
                  }
                />
              ))}
